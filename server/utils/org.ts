...
class OrgService {
  ...
  async saveSubscription(org: Org, stripeSubscription: Stripe.Subscription, product: Product) {
    const [subscription] = await db.insert(tables.subscription).values({
      ...
      expires: stripeSubscription.current_period_end as number,
    }).onConflictDoUpdate({ target: tables.subscription.stripeSubscriptionId, set: { code: product.code, name: product.name, stripeSubscriptionId: stripeSubscription.id, expires: stripeSubscription.current_period_end } }).returning()
